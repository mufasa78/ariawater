...
export const getByOrder = query({
    ...
    handler: async (ctx, { orderId }) => ctx.db.query("payments").withIndex("by_order", (q) => q.eq("orderId", orderId)).order("desc").first(),
});
...
export const getByProviderTransactionId = query({
    ...
    handler: async (ctx, { providerTransactionId }) => ctx.db.query("payments").withIndex("by_provider_transaction", (q) => q.eq("providerTransactionId", providerTransactionId)).first(),
});
export const create = mutation({
    ...
    handler: async (ctx, { orderId, provider, amount, phone }) => {
        ...
        const paymentId = await ctx.db.insert("payments", {
            orderId, provider: "lipana", amount: Math.round(amount), phone, status: "pending", createdAt: Date.now(),
        });
...
export const updateTransactionId = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.status === "failed")
            await ctx.db.patch(payment.orderId, { paymentStatus: "failed", updatedAt: Date.now() });
...
export const markSuccessful = mutation({
    ...
    handler: async (ctx, { id }) => {
        ...
        if (!payment.providerTransactionId)
            throw new ConvexError("Payment has no provider transaction ID");
        if (["successful", "failed"].includes(payment.status))
            return payment;
        const now = Date.now();
